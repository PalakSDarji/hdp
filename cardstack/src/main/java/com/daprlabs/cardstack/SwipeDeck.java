...
public class SwipeDeck extends FrameLayout {
    ...
    public SwipeDeck(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.getTheme().obtainStyledAttributes(
                ...
                0, 0);
        try {
            NUMBER_OF_CARDS = a.getInt(R.styleable.SwipeDeck_max_visible_, 3);
            ROTATION_DEGREES = a.getFloat(R.styleable.SwipeDeck_rotation_degrees_, 15f);
    ...
    private void positionItem(int index) {
     ...
     //   child.layout(left, paddingTop, left + width, paddingTop + height);
        ...
        if(index==0){
        child.layout(left, paddingTop, left + width,-10+ paddingTop + height);
         //   child.setY(-20+paddingTop);
        }else {
        child.layout(left, paddingTop, left + width, -10+paddingTop + height);
